...
export default function Edit({value,onEdit}) {
  ...
  useEffect(() => {
    return (
      setSingle(Object.assign({}, ...(value.filter((e) => e.std_id == params.id))))
    )
  }, [])

  const index=(value.findIndex((e) => e.std_id ==params.id))
      ...
      <div style={{ marginTop: "100px" }}>
        <form onSubmit={editSubmit} method="POST" style={{ width: "500px", marginRight: "auto", marginLeft: "auto" }}>
          ...
          <div className="mb-3">
            ...
            <input type="number" name='roll_no' onChange={OnChange} value={single.roll_no} className="form-control" id="b" />
          </div>
          <div className="mb-3">
            <label htmlFor="b" className="form-label">Grade</label>
            <input type="number" name='grade' onChange={OnChange} value={single.grade} className="form-control" id="b" />
          </div>
